...
class AESModeOfOperation(object):
    ...
    def convertBytes(self, bytes, start, end, mode):
    ...
    def encrypt(self, stringIn, mode, key, size, IV):
        ...
        if stringIn != None:
            for j in range(int(math.ceil(float(len(stringIn))/16))):
                ...
                plaintext = self.convertBytes(stringIn, start, end, mode)
...
def decryptData(key, data, mode=AESModeOfOperation.modeOfOperation["CBC"], unpad = True):
    ...
    if mode == AESModeOfOperation.modeOfOperation["CBC"] and unpad:
